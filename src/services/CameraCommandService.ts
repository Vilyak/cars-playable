...
export class CameraCommandService {
  ...
  enqueueShake(strength: number): void {
    this.queue.push({ type: 'shake', strength })
  }
  ...
  private apply(command: CameraCommand): void {
    ...
    switch (command.type) {
      ...
      case 'shake': {
        Camera.shakeStrength[cameraEid] = Math.max(Camera.shakeStrength[cameraEid], command.strength)
        break
      }
    }
  }
